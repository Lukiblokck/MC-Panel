...
/* ── WebSocket events ───────────────────────── */
socket.on("connect", () => {
  addLog("ok", "[Panel]", "WebSocket conectado.");
});

socket.on("disconnect", () => {
  addLog("error", "[Panel]", "WebSocket desconectado.");
  setOnline(false);
});
...
function setOnline(online) {
  ...
  label.textContent = online ? "RCON conectado" : "RCON desconectado";
}
...
function renderPlayers(players) {
  ...
  if (!players.length) {
    table.innerHTML = '<div class="empty-state">Sin jugadores online</div>';
    return;
  }
...
function clearLog() {
  ...
  addLog("gray", "[Panel]", "Consola limpiada.");
}
...
async function post(url, body = {}) {
  try {
    ...
    return { ok: false, response: "Error de red: " + e.message };
  }
}
...
async function quickAction(action) {
  switch (action) {
    ...
    case "kickall": {
      const reason = prompt("Razón del kick:", "Reinicio del servidor");
...
async function sendWorldCmd() {
  ...
  el.textContent = res ? res.response : "Sin respuesta";
}

/* ── Player actions ─────────────────────────── */
async function kickPlayer(name) {
  const reason = prompt(`Razón para kickear a ${name}:`, "Kicked by admin");
  if (reason === null) return;
...
async function banPlayer(name) {
  ...
  const reason = prompt("Razón del ban:", "Banned by admin");
  if (reason === null) return;
...
addLog("gray", "[Panel]", "Conectando al servidor...");
...
(function initTheme() {
  ...
  toggle.addEventListener("change", () => {
    ...
    addLog("gray", "[Panel]", theme === "day" ? "Modo día activado." : "Modo noche activado.");
  });
})();